...
public class PivotInit extends Command {
  ...
  private static final double PIVOT_HOME_TIMEOUT_SECONDS = 5.0;
  ...
  public void execute() {
    if (!isOverLoaded) {
      ...
      isOverLoaded =
          (torque_current > Constants.IntakePID.pivotThresholdCurrentAmps) && (velocity < 0.015)
              || (Timer.getFPGATimestamp() - pivotHomeStartSec > PIVOT_HOME_TIMEOUT_SECONDS);
    } else {
      isHomed = true;
    }
  }
